...
template <bool Dynamic> class BinaryWriter_ : public Writer {
public:
  using data_t = std::conditional_t<Dynamic, std::vector<std::uint8_t>,
                                    mct::vector<std::uint8_t>>;
  BinaryWriter_(data_t &data, bool trivial_as_binary = true)
      : Writer(trivial_as_binary), data(data), pos(data.size()),
        binary_depth(false), binary_start(0), trivial_list_length(0) {}
  ...
  void binary(const std::uint8_t *input_data, std::size_t length,
              std::size_t stride, bool fixed_length) override;
  ...
  std::span<std::uint8_t> result() const { return std::span(&data[0], pos); }
...
template <writeable T> std::vector<std::uint8_t> write_binary(const T &value) {
