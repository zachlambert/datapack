...
struct json_functions<bool> {
    static bool from_json(datapack::JsonConstNode node, bool& value) {
...
struct json_functions<T> {
    static bool from_json(datapack::JsonConstNode node, T& value) {
...
struct json_functions<std::vector<T>> {
    ...
    static void to_json(const std::vector<T>& vector, datapack::JsonNode node) {
        static_assert(writable_json<T>);
        node.set_array();
...
struct json_functions<std::unordered_map<std::string, T>> {
    static bool from_json(datapack::JsonConstNode node, std::unordered_map<std::string, T>& map) {
    ...
    static bool to_json(const std::unordered_map<std::string, T>& map, datapack::JsonNode node) {
        node.set_object();
...
struct json_functions<std::map<std::string, T>> {
    bool from_json(datapack::JsonConstNode node, std::map<std::string, T>& map) {
        if (!node.is_object()) return false;
        bool valid = true;
        for (auto iter = node.begin(); iter != node.end(); iter++) {
            valid &= (*iter).read(map[std::string(iter.key())]);
    ...
    bool to_json(const std::map<std::string, T>& map, datapack::JsonNode node) {
        node.set_object();
        for (const auto& pair: map) {
            node[pair.first] = pair.second;
        }
