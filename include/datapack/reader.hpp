...
public:
    Packer(
        bool trivial_as_binary=false,
        bool is_tokenizer=false,
        bool check_constraints=false
    ):
        valid_(true),
        trivial_as_binary_(trivial_as_binary),
        is_tokenizer_(is_tokenizer),
        check_constraints_(check_constraints),
        constraint_(nullptr)
    {}
    ...
    virtual std::tuple<const std::uint8_t*, std::size_t> binary(std::size_t length, std::size_t stride) = 0;
